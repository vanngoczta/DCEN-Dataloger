...
sys.path.append('/home/pi/Pump/gsmmodem')
...
class SerialComms(object):
    ...
    def __init__(self, port, baudrate=115200, notifyCallbackFunc=None, smsReceivedCallbackFunc=None, fatalErrorCallbackFunc=None, *args, **kwargs):
        """ Constructor
    ...
    def connect(self):
        print(" Connects to the device and starts the read thread " )               
    ...
    def setsms(self):
        ...
        if self._smsReadSupported:
            try:
                self.write('AT+CNMI=2,1,0,2\r') # Set message notifications
            except CommandError:
    ...
    def sendUssd(self, ussdString, responseTimeout=15):
        ...
        try:
            cusdResponse = self.write('AT+CUSD=1\r,"{0}",15'.format(ussdString),waitForResponse=True, timeout=responseTimeout) # Should respond with "OK"
        except Exception:
    ...
    def readStoredSms(self, index, memory=None):
        ...
        msgData = self.write('AT+CMGR={0}\r'.format(index))
...
class HuaweiModem(object):
    ...
    def GetAllSMS(self):
        ...
        self.ser.read(100)
        self.ser.write('AT+CPBS="SM"\r\n')
        time.sleep( 1)   
        self.ser.read(100)
        self.ser.write('AT+CPMS="SM","SM","SM"\r\n')
        time.sleep( 1) 
        self.ser.read(100)
        print ("2")
