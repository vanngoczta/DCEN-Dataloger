    ...
    with sqlite3.connect(dbname) as conn:
        ...
        for row in curs.fetchmany(Set.maxchannel):
            ...
            if(Set.functionchannel[indexmodbus]==19):
                try:
                    usbrtu.readdata[indexmodbus]=float(row[4])
                    ...
                    print("Read Total Flow:"+str(usbrtu.readdata[indexmodbus]))
                except Exception as e:
                    usbrtu.readdata[indexmodbus]=0
    ...
    try:
        ...
        for files in glob.glob(file_txt_csv):
            ...
            if(file_txt_csv=="*.txt"):      #KCNDiemThuy20170228145700.txt
                dayftp="ngay "+str(files[(length-18):(length-16)])
                monthftp="thang "+str(files[(length-14):(length-12)])
                yearftp="Nam "+str(files[(length-18):(length-14)])
                ...
                dayftp="ngay "+str(files[(length-12):(length-10)])
                monthftp="thang "+str(files[(length-8):(length-6)])
                yearftp="Nam "+str(files[(length-12):(length-8)])
            ...
            #fileupload=open(path_file_ftp,'rb')
            #print(fileupload.readlines())
            Gsm.reportsms=files+" "+ftp.storlines('STOR '+files ,open(path_file_ftp,'rb'))
            #fileupload.close()
            print("Storlines FTP12:"+Gsm.reportsms+"index cwd:"+str(total_cwd))
            for index_cdup in range (total_cwd+2):
                ftp.sendcmd('cdup')
            #print("List: "+ftp.retrlines('LIST'))    #list subdirectory contents
            # Xoa file
            os.remove(path_file_ftp)
    ...
    try:
        ...
        # Doc gia tri do ADC0
            ...
            if(Set.startreg[indexmodbus]==0):
                # Loc gia tri ve 0
                adcread1=ads0.analogRead(indexads)
                ...
                # Loc gia tri ve 0
                adcvolt1=ads0.analogReadVolt(indexads)   #1mA*200ohm=200mV
        ...
        # Doc gia tri do ADC1
            ...
            if(Set.startreg[indexmodbus]==0):
                # Loc gia tri ve 0
                adcread1=ads1.analogRead(indexads)
                adcread=Fir.minmaxmedium(adcread1,indexmodbus)
                if(adcread==0 and usbrtu.readdata[indexmodbus]>0):
                    usbrtu.counterror[indexmodbus]+=1
                    if(usbrtu.counterror[indexmodbus]>=10):
                ...
                # Loc gia tri ve 0
                adcvolt1=ads1.analogReadVolt(indexads)   #1mA*200ohm=200mV
                adcvolt=Fir.minmaxmedium(adcvolt1,indexmodbus)
                if(adcvolt==0 and usbrtu.readdata[indexmodbus]>0):
                    usbrtu.counterror[indexmodbus]+=1
                    if(usbrtu.counterror[indexmodbus]>=10):
