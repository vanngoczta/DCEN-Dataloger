...
# Greentech
version="Ver3:DA.FC.SMS.CAM0718"    
...
# Time | Value
# 17/03/18 update historytrend theo json highchart
@webiopi.macro
def load_history_days_channel(channel,days):
    ...
    if(newdatahistory==1 or len(daydatachart)==0):
        ...
        with sqlite3.connect(dbname,isolation_level=None) as conn:
            ...
            if(len(days)>0):
                value=[]
                curs.execute("SELECT * FROM historydata WHERE channel='%s' AND tdate>date('now','localtime','-%s day')  ORDER BY tdate ASC, ttime ASC" % (channel,days))
                for row in curs.fetchmany(1000):
                    d=row[1]+' '+row[2]
                    dates = datetime.datetime.strptime(d, '%Y-%m-%d %H:%M:%S')
                    valueandtime=[]
                    timetuple=int((time.mktime(dates.timetuple())+25200)* 1000)
                    valueandtime.append(timetuple)
                    valueandtime.append(row[4])
                    value.append(valueandtime)
                datas = dict()
                datas['name'] = Set.namechannel[row[3]]
                datas['data'] = value
                context={
                    'units': Set.unitreg[row[3]],
                    'datas':datas,
                }         
...
def load_history_date_channel(channel,startdate,enddate):
    ...
    if(newdatahistory==1 or len(datedatachart)==0):
        ...
        with sqlite3.connect(dbname,isolation_level=None) as conn:
            ...
            if(len(startdate)>0 and len(enddate)>0):
                value=[]
                curs.execute("SELECT * FROM historydata WHERE channel='%s' AND tdate>='%s' AND tdate<='%s'  ORDER BY ID ASC" % (channel,startdate,enddate))
                for row in curs.fetchmany(1000):
                    d=row[1]+' '+row[2]
                    dates = datetime.datetime.strptime(d, '%Y-%m-%d %H:%M:%S')
                    valueandtime=[]
                    timetuple=int((time.mktime(dates.timetuple())+25200)* 1000)
                    #print(timetuple)
                    #timetuple=int(dates.timestamp()+25200)* 1000
                    valueandtime.append(timetuple)
                    valueandtime.append(row[4])
                    value.append(valueandtime)
                datas = dict()
...
def UpdateStatus():
    ...
    if(IOsetting.test_ds1307>0):
        data=data+Netsetting.reportserver[:60]+". Error DS1307,"+ Rea.reportmodbus[:50]+"\r\n"
    else:
...
class netsetting(object):
    ...
    def Load_setting(self):     
        ...
        if(len(setting)>=38):
            ...
            # 15/05/18
            self.tel[0]=setting[9]
            self.tel[1]=setting[10]
            self.tel[2]=setting[11]
            self.tel[3]=setting[12]
            self.tel[4]=setting[13]
            
            self.serverftp=setting[14]
            self.pathftp=setting[15]
            self.userftp=setting[16]
            self.passftp=setting[17]
            self.username=setting[18]
            self.newpass=setting[19]
            self.conpass=setting[20]
            # Update 29/12/16
            self.username2=setting[21]
            self.newpass2=setting[22]
            self.conpass2=setting[23]
            
            self.mailserver=setting[24]
            self.mailport=setting[25]
            self.mailfrom=setting[26]
            self.mailpass=setting[27]
            self.mailto0=setting[28]
            self.mailto1=setting[29]
            self.mailto2=setting[30]

            self.serverftp2=setting[31]
            self.pathftp2=setting[32]
            self.userftp2=setting[33]
            self.passftp2=setting[34]
            self.filenameftp=setting[35]
            self.ssid=setting[36]
            self.psk=setting[37]
    ...
    def getiface(self,neteth0orwlan0):
        #neteth0orwlan0='eth0' hoac neteth0orwlan0='wlan0'
        '''ni.ifaddresses('eth0')
        ...
        print(ni.ifaddresses(neteth0orwlan0))
        self.ip=ni.ifaddresses(neteth0orwlan0)[2][0]['addr']
        self.netmask=ni.ifaddresses(neteth0orwlan0)[2][0]['netmask']
        #{'default': {2: ('192.168.1.1', 'eth0')}, 2: [('192.168.1.1', 'eth0', True)]}
        self.gateway=ni.gateways()[2][0][0]
        self.mac=ni.ifaddresses(neteth0orwlan0)[17][0]['addr']
    ...
    def setiface(self):
        ...
        with open(file,"r+") as fh:
            ...
            contentnew='interface wlan0'+'\nstatic ip_address='+self.ip+'\nstatic routers='+self.gateway+'\nstatic domain_name_servers=8.8.8.8 8.8.4.4'
...
class iosetting(object):
    def __init__(self):
        ...
        self.calib_status_channel=[0,0,0,0,0,0,0,0]
        self.modeinput=[2,2,2,2,2,2,2,2]
        self.modeoutput=[0,0,0,0,0,0,0,0]   #=0 che do tu dong, =1 che do bang tay
        self.lowinput=["Open IN1","Open IN2","Open IN3","Open IN4",\
                       "Open IN5","Open IN6","Open IN7","Open IN8"]
        self.highinput=["Close IN1","Close IN2","Close IN3","Close IN4",\
                        "Close IN5","Close IN6","Close IN7","Close IN8"]
    ...
    def Load_setting(self):
        ...
        if(len(setting)>=52):
            try:
                ...
                self.CaL_status=int(setting[2])
                self.tinfor=int(setting[3])     #Hour
                self.tdelay=int(setting[4])     #Min
                self.tsiren3=int(setting[5])     #Sec
                self.channel3highon5=int(setting[6] )         #Min
                self.tonout6=int(setting[7] )         #Sec
                self.tloopout12=int(setting[8] )     #Min
                self.modeoutgen=int(setting[9])      #auto=0, manual=1
                self.temphighon12=float(setting[10])    #oC
                self.templowoff12=float(setting[11])    #oC
                self.channel2highon4=int(setting[12])     #%
                ...
                print ("Error IO Setting:", e)
                flag_error=1
        if(flag_error==1 or len(setting)<51):
            data=""
            data=str(self.alarm)+"\n"+str(self.sms)+"\n"+str(self.CaL_status)+"\n"+str(self.tinfor)+"\n"+str(self.tdelay)+"\n"+str(self.tsiren3)+"\n"+str(self.channel3highon5)+"\n"+str(self.tonout6)+"\n"+\
                  str(self.tloopout12)+"\n"+str(self.modeoutgen)+"\n"+str(self.temphighon12)+"\n"+str(self.templowoff12)+"\n"+str(self.channel2highon4)+"\n"
            for i in range(len(self.modeinput)):  
        ...
        if(len(setting)>=6+MAXCHANNEL*9):
            ...
            for i in range(MAXCHANNEL):    #6+MAXCHANNEL*9
                ...
                if(self.functionchannel[i]==6 and (self.typereg[i]==1 or self.typereg[i]==3)):
                    Rea.writefc6[i]=1
                    Rea.writedatafc6[i]=str(self.lowset[i])
...
def handleSms(sms):
    ...
    if(now.year==1970):
        # Read time DS1703
        os.system("sudo date -s '"+str(Gsm.timesmsrec)+"'")
    ...
    if(sms.text.find("Alarm off")!=-1):
        ...
        data=data+"IP: "+str(Netsetting.ip)+":8880\n"
        for i in range(Set.maxchannel):
            data=data+str(Set.namechannel[i])+":"+str(CaL.ReV[i])+" "+str(Set.unitreg[i])+"\n"
...
class camerasetting(object):
    def __init__(self):
        self.file = "/home/pi/bts/Camsetting.txt"
        open(self.file, "r")
        self.counterror=[0,0]
        self.timemin=0
        self.indexcam=0
    ...
    def Load_setting(self):     
        ...
        strdata=str(data)
        setting=strdata.split('\n')
        print("Camera Setting:",setting)
        if(len(setting)>=12):
            self.ipc[0]=setting[0]
            self.portc[0]=setting[1]
            self.urlc[0]=setting[2]
            self.userc[0]=setting[3]
            self.passc[0]=setting[4]
            self.modec[0]=setting[5]
            self.ipc[1]=setting[6]
            self.portc[1]=setting[7]
            self.urlc[1]=setting[8]
            self.userc[1]=setting[9]
            self.passc[1]=setting[10]
            self.modec[1]=setting[11]
            #http://admin:xxxx@192.168.0.xx/ISAPI/Streaming/channels/101/picture
            if(len(self.userc[0]) and len(self.portc[0]) and len(self.passc[0]) and len(self.ipc[0]) and len(self.urlc[0])):
                self.pathurl[0]='http://'+self.userc[0]+':'+self.passc[0]+'@'+self.ipc[0]+':'+self.portc[0]+'/'+self.urlc[0]
                print(self.pathurl[0])
            else:
                self.pathurl[0]=""
            if(len(self.userc[1]) and len(self.portc[1]) and len(self.passc[1]) and len(self.ipc[1]) and len(self.urlc[1])):   
                self.pathurl[1]='http://'+self.userc[1]+':'+self.passc[1]+'@'+self.ipc[1]+':'+self.portc[1]+'/'+self.urlc[1]
                print(self.pathurl[1])
            else:
                self.pathurl[1]=""
            print("Read Cam Setting ok")
            data=str(self.ipc[0])+"\n"+str(self.portc[0])+"\n"+str(self.urlc[0])+"\n"+str(self.userc[0])+"\n"+str(self.passc[0])+"\n"+str(self.modec[0])+"\n"
            data=data+str(self.ipc[1])+"\n"+str(self.portc[1])+"\n"+str(self.urlc[1])+"\n"+str(self.userc[1])+"\n"+str(self.passc[1])+"\n"+str(self.modec[1])+"\n"
        else:
            data=str(self.ipc[0])+"\n"+str(self.portc[1])+"\n"+str(self.urlc[0])+"\n"+str(self.userc[0])+"\n"+str(self.passc[0])+"\n"+str(self.modec[0])+"\n"
            data=data+str(self.ipc[1])+"\n"+str(self.portc[1])+"\n"+str(self.urlc[1])+"\n"+str(self.userc[1])+"\n"+str(self.passc[1])+"\n"+str(self.modec[1])+"\n"
            self.Save_setting(data)
            print("Default Cam Setting ok")
        return (data)
    ...
    def Save_setting(self,data):
        ...
        if(len(setting)>=12):
            ...
            self.passc[1]=setting[10]
            self.modec[1]=setting[11]
            print("Saved Cam Setting")
            return (data)

    
# -----Cai dat Camera ip----
Camerasetting=camerasetting()
Camerasetting.Load_setting()
# Phan web truy cap
@webiopi.macro
def load_cameraipsetting():
    data=Camerasetting.Load_setting()
    return (data)

@webiopi.macro
def save_cameraipsetting(data):
    read=urllib.parse.unquote(data)
    read=read.replace(";", "\n")
    read=read.replace(':', '/')
    read=read.replace('~', '#')
    Camerasetting.snapshot[0]=1
    Camerasetting.snapshot[1]=1
    return (Camerasetting.Save_setting(read))

# Danh cho canh bao chup anh
def Alarm_snapshot_camera(modecam):
    for indexcami in range(2):
        modecamsetting=int(Camerasetting.modec[indexcami])
        if(modecam>0 and (modecam == modecamsetting or modecamsetting == 3)):
            pathpicture="/home/pi/bts/upload/picture_alarm/"+str(database_script.namepicture)+str(indexcami)+".jpg"
            path_file_download=str(Netsetting.ip)+":8880/upload/picture_alarm/"+str(database_script.namepicture)+str(indexcami)+".jpg"
            if(Camerasetting.camconnect[indexcami]==1 and len(Camerasetting.pathurl[indexcami]) and indexcami<2):
                Camerasetting.snapshot[indexcami]=0
                try:
                    download(Camerasetting.pathurl[indexcami], pathpicture)
                    #if(Camerasetting.camconnect[indexcami]==0):
                    Netsetting.reportserver="Snapshot alarm "+str(indexcami+1)+':'+Camerasetting.ipc[indexcami]
                    print(Netsetting.reportserver)
                    Camerasetting.camconnect[indexcami]=1
                    #MAIL_Attachment(pathpicture,path_file_download)
                except Exception as e:
                    Netsetting.reportserver="Alarm Camera "+str(indexcami+1)+':'+str(e)
                    print(Netsetting.reportserver)
                    Camerasetting.camconnect[indexcami]=0
                    pass
...
def setup():
    ...
    #if(now.year==1970):
    # Read time DS1703
    IOsetting.test_ds1307=os.system("sudo hwclock -s")
...
def inputEventHandler(pin):
    ...
    if(Set.functionchannel[0]==23):
        ...
        # Bao trang thai hieu chuan cho cac kenh do =1 dang hieu chuan, =0 binh thuong
        if(pin>=INP1 and pin<=INP6):
            if(IOsetting.modeinput[pin-INP1]==3):
                time.sleep(0.5)
                if(GPIO.input(pin)==1):
                    IOsetting.calib_status_channel[pin-INP1]=1
                elif(GPIO.input(pin)==0):
                    IOsetting.calib_status_channel[pin-INP1]=0
                IOsetting.Beep=1  
        # Trang thai tat bao dong
        if(IOsetting.alarm==0):
            return
        if(pin>=INP1 and pin<=INP6):
            if(IOsetting.modeinput[pin-INP1]==0 and GPIO.input(pin)==0):
                stralarm=str(pin-INP1+1)+": "+str(IOsetting.lowinput[pin-INP1])
                q.put(stralarm)
                Insertalarmsql(stralarm)
                Alarm_snapshot_camera(1)
                ...
                stralarm=str(pin-INP1+1)+": "+str(IOsetting.highinput[pin-INP1])
                q.put(stralarm)
                Insertalarmsql(stralarm)
                Alarm_snapshot_camera(1)
                if(IOsetting.sireninput[pin-INP1]==1):
                ...
                if(GPIO.input(pin)==1):
                    stralarm=str(pin-INP1+1)+str(GPIO.input(pin))+": "+str(IOsetting.highinput[pin-INP1])
                    q.put(stralarm)
                    Insertalarmsql(stralarm)
                    Alarm_snapshot_camera(1)
                    ...
                    stralarm=str(pin-INP1+1)+str(GPIO.input(pin))+": "+str(IOsetting.lowinput[pin-INP1])
                    q.put(stralarm)
                    Insertalarmsql(stralarm)
                    Alarm_snapshot_camera(1)
            ...
            for i in range(0,8):
                if(Set.functionchannel[i]==10):
                    ...
                    if(data[fieldstr]!=None):
                        ...
                        Netsetting.reportserver=fieldstr+": "+data[fieldstr]+" "+str(hour)+":"+str(minute)
                        if(totalmin1>totalmin2-10 and totalmin1<totalmin2+10):
...
try:
    ...
    Netsetting.reportserver="Database empty!"
...
def decode(s):
    return s\
        ...
        .replace('\\t','\t')\
#----------------------------
def Insertalarmsql(alarstring):
    global RamAlarmList
    try:
        database_script.insert_alarm_tablet(alarstring)       # hay bi loi database dan den treo thiet bi
        RamAlarmList=""
...
def loop():
    ...
    try:
        ...
        if(now.minute!=lastminute):
            ...
            # TRUY CAP WEB ECAPRO
            try:
                req = Request('http://ecapro.com.vn/vi/tin-tuc/tin-tc/262-quantracmoitruong', headers={'User-Agent': 'Mozilla/5.0'})
            ...
            TempCPU = round(int(open('/sys/class/thermal/thermal_zone0/temp').read()) / 1e3, 1) # Get Raspberry Pi CPU temp
            ...
            if(countmin >= Set.tupload):
                ...
                if(len(Netsetting.serverftp)>0 and len(Netsetting.userftp)>0 and len(Netsetting.passftp)>0 and len(Netsetting.filenameftp)):
                    ...
                    statussensor=0
                    for i in range(Set.maxchannel):
                        if(i<6 and IOsetting.CaL_status==0 and IOsetting.calib_status_channel[i]==1):
                            statussensor=1
                        elif(IOsetting.CaL_status==1):
                            statussensor=1
                        elif(Rea.status[i]>=1):
                            statussensor=0
                        elif(Rea.status[i]==0):
        ...
        if(IOsetting.alarm and not q.empty()):
            ...
            try:
                ...
                pass
            #5 so dien thoai
            for i in range(len(Netsetting.tel)):    
        ...
        # Doc gia tri do ADC1
            ...
            if(Set.startreg[Idx]==0):
                # Loc gia tri ve 0, gia tri ve -3 ~ -5 khi không co dong
                ...
                # Loc gia tri ve 0, gia tri ve 0.0025 khi khong co dong
        ...
        for i in range(Set.maxchannel):
            ...
            # Gia tri binh thuong
                ...
                if(Rea.status[i]>1):
                    Rea.status[i]=1
                if(self.Tdelay[i]>5):
                    stralarm=str(Set.namechannel[i])+": "+str(CaL.ReV[i])+" "+str(Set.unitreg[i])
                    q.put(stralarm)
                    Insertalarmsql(stralarm)
                    Alarm_snapshot_camera(2)
        ...
        try:
            ...
            if((functionchannel==6 or functionchannel==16) and self.writefc6[indexaddress]==1 and len(self.writedatafc6[indexaddress])):     
                if(typereg==1):
                    ...
                    self.instrument.write_register(startreg,float(self.writedatafc6[indexaddress]),numberreg,functionchannel,True)
...
def polling_thread():
    ...
    # polling loop
    while True:
        #Ket noi canh bao camera1 mat ket noi
        if(Camerasetting.indexcam==0):
            Camerasetting.indexcam=1
        else:
            Camerasetting.indexcam=0
        #time.sleep(1)
        if(len(Camerasetting.pathurl[Camerasetting.indexcam])
           and int(Camerasetting.modec[Camerasetting.indexcam])>0 ):
            #print("Thread cam\n")
            Camerasetting.snapshot[Camerasetting.indexcam]=0
            pathpicture="/home/pi/bts/cam"+str(Camerasetting.indexcam+1)+".jpg"
            try:
                #create the object, assign it to a variable
                # make a request
                download(Camerasetting.pathurl[Camerasetting.indexcam], pathpicture)
                if(Camerasetting.camconnect[Camerasetting.indexcam]==0):
                    Netsetting.reportserver="Snapshot update "+str(Camerasetting.indexcam+1)+':'+Camerasetting.ipc[Camerasetting.indexcam]
                    print(Netsetting.reportserver)
                Camerasetting.camconnect[Camerasetting.indexcam]=1
                Camerasetting.counterror[Camerasetting.indexcam]=0
            except Exception as e:
                Netsetting.reportserver="Camera "+str(Camerasetting.indexcam+1)+':'+str(e)
                print(Netsetting.reportserver)
                Camerasetting.counterror[Camerasetting.indexcam]=Camerasetting.counterror[Camerasetting.indexcam]+1
                if(Camerasetting.camconnect[Camerasetting.indexcam]==1
                   and IOsetting.alarm==1
                   and Camerasetting.counterror[Camerasetting.indexcam]>=10):
                    Camerasetting.counterror[Camerasetting.indexcam]=0
                    q.put(str(Netsetting.reportserver)+"\n"+str(Camerasetting.ipc[Camerasetting.indexcam]))
                    Insertalarmsql(str(Netsetting.reportserver)+"\n"+str(Camerasetting.ipc[Camerasetting.indexcam]))
                Camerasetting.camconnect[Camerasetting.indexcam]=0
                pass
            time.sleep(0.2)
    ...
    while True:
        ...
        if(ads0_analog==True):
            ...
            if(indexadc0>=4):
                indexadc0=0
                print("Read Adc0\n",valueadc0)
                if (valueadc0[0] >= 32760 or valueadc0[1] >= 32760 or valueadc0[2] >= 32760 or valueadc0[3] >= 32760):
                    print("Reset Adc0\n")
                    IOsetting.reporthmi="ADC0: Over 20mA"
                    countoveradc0=countoveradc0+1
                    if(countoveradc0>=20):
        ...
        if(ads1_analog==True):
            ...
            if(indexadc1>=4):
                indexadc1=0
                print("Read Adc1\n",valueadc1)
                if (valueadc1[0] >= 32760 or valueadc1[1] >= 32760 or valueadc1[2] >= 32760 or valueadc1[3] >= 32760):
                    print("Reset Adc1\n")
                    IOsetting.reporthmi="ADC1: Over 20mA"
                    countoveradc1=countoveradc1+1
                    if(countoveradc1>=20):
                        os.system("sudo /etc/init.d/webiopi restart")
