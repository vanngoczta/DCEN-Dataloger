...
global datedatachart,daydatachart,lastchannel,countminsql,lastminutesql
inputstatus=0
outputstatus=0
modeoutputstatus=0
Alarmspk=0
...
def UpdateStatus():
    ...
    data=data+Netsetting.reportserver[:20]+","+IOsetting.reporthmi[:20]+". "+ usbrtu.reportmodbus[:40]+"\r\n"
    return(data)
...
def loop():
    ...
    if(now.second!=lastsecond):
        ...
        # Bao dong ra loa cho cac module ECA-TH485MOD    
        for i in range(MAXCHANNEL):
            if(alarmth.Siren[i]>0):
                alarmth.Siren[i]=alarmth.Siren[i]-1
...
class AlarmEvents(object):
    def __init__(self):
        ...
        for i in range(MAXCHANNEL):
            ...
            self.Siren.append(0)            #0 Normal, >0 Alarm
        ...
        for i in range(Set.maxchannel):
            ...
            if(Calibration.realvalue[i]>float(Set.highset[i]) and self.FlagEventTH[i]!=1):
                ...
                if(self.Tdelay[i]>2):
                    ...
                    usbrtu.status[i]=3
                    IOsetting.tsiren3buff=IOsetting.tsiren3
                    self.Siren[Set.addchannel[i]]=IOsetting.tsiren3
                    self.Tdelay[i]=0
                    indextask=4.0
...
class ModbusRTU(object):
    ...
    def readmodbus(self,indexaddress,address,functionchannel,startreg,numberreg,typereg):
        ...
        try:
            ...
            elif(functionchannel==5):
                if(alarmth.Siren[address]>0):
                    ...
                    print("write_bit=0")
                self.status[indexaddress]=1
                self.data=alarmth.Siren[address]
                #self.reportmodbus="Modbus:"+str(address)+"+"+str(startreg)+"+"+str(functionchannel)+"="+str(self.data)
                #return 1
