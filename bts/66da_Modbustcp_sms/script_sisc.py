...
global connect485,indexvalue,counthour
inputstatus=0
outputstatus=0
modeoutputstatus=0
Alarmspk=0
...
def load_history_data_channel_day2(channel,days):
    ...
    if(len(days)>0):
        for row in curs.execute("SELECT * FROM historydata WHERE channel='%s' AND tdate>date('now','localtime','-%s day')  ORDER BY ID DESC" % (channel,days)):
            data=data+str(i+1)+","+str(row[1])+","+str(row[2])+","+str(row[3]+1)+","+ Set.namechannel[row[3]] + "," + str(row[4])+","+Set.unitreg[row[3]]+","+str(row[5])+"\r\n"
            i=i+1
            if(i>5000):
                break                
...
class netsetting(object):
    def __init__(self):
        ...
        self.mailto0="trainer.ecapro@gmail.com"
        self.mailto1=""
        self.mailto2=""
...
def handleSms(sms):
    ...
    if(sms.text.find("Alarm off")!=-1):
        ...
        data=str(Set.namechannel[0])+":"+str(Calibration.realvalue[0])+" "+str(Set.unitreg[0])+"\n"+str(Set.namechannel[1])+":"+str(Calibration.realvalue[1])+" "+str(Set.unitreg[1])+"\n"+\
              str(Set.namechannel[2])+":"+str(Calibration.realvalue[2])+" "+str(Set.unitreg[2])+"\n"+str(Set.namechannel[3])+":"+str(Calibration.realvalue[3])+" "+str(Set.unitreg[3])+"\n"+\
              str(Set.namechannel[4])+":"+str(Calibration.realvalue[4])+" "+str(Set.unitreg[4])+"\n"+str(Set.namechannel[5])+":"+str(Calibration.realvalue[5])+" "+str(Set.unitreg[5])+"\n"+\
              str(Set.namechannel[6])+":"+str(Calibration.realvalue[6])+" "+str(Set.unitreg[6])+"\n"+str(Set.namechannel[7])+":"+str(Calibration.realvalue[7])+" "+str(Set.unitreg[7])+"\n"+\
              str(Set.namechannel[8])+":"+str(Calibration.realvalue[8])+" "+str(Set.unitreg[8])+"\n"+str(Set.namechannel[9])+":"+str(Calibration.realvalue[9])+" "+str(Set.unitreg[9])
        textsms=Netsetting.hostname+'\n'+data
        ...
        data=data+str(Set.namechannel[0])+":"+str(Calibration.realvalue[0])+" "+str(Set.unitreg[0])+"\n"+\
                str(Set.namechannel[1])+":"+str(Calibration.realvalue[1])+" "+str(Set.unitreg[1])+"\n"+\
                str(Set.namechannel[2])+":"+str(Calibration.realvalue[2])+" "+str(Set.unitreg[2])+"\n"+\
                str(Set.namechannel[3])+":"+str(Calibration.realvalue[3])+" "+str(Set.unitreg[3])+"\n"
...
def loop():
    ...
    if(now.year>2016 and now.month > 5):
        return
    looprunning=1
    indextask=0.1 
    # Kiem tra theo giay
    ...
    #for i in range(Set.maxchannel):
    indextask=2.1   
    if(countmin==0 and indexvalue<Set.maxchannel):
        print("Write data to sql:",indexvalue)
        try:
            database_script.insert_history_data(indexvalue,Calibration.realvalue[indexvalue],usbrtu.status[indexvalue])
            if(Netsetting.reportserver.find("Error database:")!=-1):
                Netsetting.reportserver="Written database:"+str(indexvalue+1)
        except Exception as e:
            Netsetting.reportserver="Error database:"+str(indexvalue+1)+str(e)
            print(Netsetting.reportserver)
    ...
    try:
        ...
        Calibration.realvalue[indexmodbus]=round((Fir.update(usbrtu.readdata[indexmodbus],indexmodbus)*Calibration.gain[indexmodbus]+Calibration.offset[indexmodbus]),2)    #Calibration.gain[indexmodbus] Calibration.offset[indexmodbus]
        ...
        for i in range(Set.maxchannel):
            ...
            if(Calibration.realvalue[i]>float(Set.highset[i]) and self.FlagEventTH[i]!=1):
                ...
                if(self.Tdelay[i]>10):
                    ...
                    self.Tdelay[i]=0
                    indextask=4.1
            elif(Calibration.realvalue[i]>=float(Set.lowset[i])+1 and Calibration.realvalue[i]<=float(Set.highset[i])-1 and self.FlagEventTH[i]>0):
                self.Tdelay[i]=self.Tdelay[i]+1
                if(self.Tdelay[i]>2):
...
class ModbusRTU(object):
    ...
    def readmodbus(self,indexaddress,address,functionchannel,startreg,numberreg,typereg):
        ...
        try:
            ...
            #self.open()
            self.reportmodbus="Error Modbus:"+str(address)+"+"+str(startreg)+"+"+str(functionchannel)+"="+str(self.data)+"Co:"+str(self.counterror[indexaddress])
...
def blink():
    ...
    while True:
        ...
        if(looprunning==1 and now.minute==0): #60 phut kiem tra 1 lan
            ...
            textsms=textsms+strftime("%H:%M:%S,%d/%m/%y",localtime())+"\n"+str(Netsetting.ip)+":8880"
            Netsetting.reportserver="Error SysNet: "+str(indextask)
            try:
                MAIL(textsms)
                #Netsetting.reportserver="Sent Mail"   #Khac phuc loi khi nhan tin het tien.
            except:
