...
global datedatachart,daydatachart,lastchannel,Idx,timesecnow
...
def Output(index):
    ...
    for i in range(Set.maxchannel):
        ...
        data=data+Set.unitreg[i] +","+ str(Rea.status[i])+","+str(inputstatus)+","+str(outputstatus)+"\r\n"
    ...
    with sqlite3.connect(dbname) as conn:
        ...
        for row in curs.fetchmany(Set.maxchannel):
            ...
            if(Set.functionchannel[Idx]==19):
                try:
                    Rea.Val[Idx]=float(row[4])
                    CaL.ReV[Idx]=round((Rea.Val[Idx]*CaL.gain[Idx]+CaL.offset[Idx]),2)
                    print("Read Total Flow:"+str(Rea.Val[Idx]))
                except Exception as e:
                    Rea.Val[Idx]=0
...
def loop():
    ...
    global indexvalue,counthour,Idx
    ...
    try:
        ...
        if(Set.functionchannel[Idx]<=7):
            if(len(CaL.function[Idx])==0):
                CaL.ReV[Idx]=round((Rea.Val[Idx]*CaL.gain[Idx]+CaL.offset[Idx]),2)
            else:
                try:
                    exec(CaL.function[Idx])
                    Rea.reportmodbus=str(CaL.ReV[Idx])+"="+str(CaL.function[Idx])
                except Exception as e:
                    Rea.reportmodbus="Error Functions Channel: "+str(Idx+1)+" "+str(e)            
        time.sleep(Set.timeout)
        print(Rea.reportmodbus) 
        if(Rea.status[Idx]>0):
...
def blink():
    global Alarmspk,Beep,looprunning,indextask,valueadc0,valuevolt0,valueadc1,valuevolt1
    lastminute=0
    count=0
    indexadc0=0
    indexadc1=0
