...
MAXCHANNEL=21
# Variable global
global inputstatus,outputstatus,modeoutputstatus,Alarmspk,countsec,countmin,Beep,lastsecond,lastminute,lasthour,datetimealarms,looprunning,indextask
global connect485,indexvalue,counthour,datedatahistory,daydatahistory,newdatahistory
global datedatachart,daydatachart,lastchannel,ads0_analog,ads1_analog,indexmodbus,timesecnow
timesecnow=0
indexmodbus=0
inputstatus=0
outputstatus=0
...
def UpdateMonitor():
    ...
    #    looprunning=0
    for i in range(Set.maxchannel):
        if(i==4 and Set.startreg[i]==8 and usbrtu.readdata[i]>=10000):
            valuemodbus=round((Calibration.realvalue[i]/1000),3)
            Set.unitreg[i]='ms/Cm'
        elif(i==4 and Set.startreg[i]==8 and usbrtu.readdata[i]<10000):
            valuemodbus=round(Calibration.realvalue[i],1)
            Set.unitreg[i]='us/Cm'
        else:
            valuemodbus=Calibration.realvalue[i]
        data=data+strftime("%H:%M:%S",localtime())+","+str(i+1)+"," + Set.namechannel[i] +","
...
def UpdateStatus():
    ...
    data=data+Netsetting.reportserver[:60]+","+IOsetting.reporthmi[:30]+". "+ usbrtu.reportmodbus[:50]+"\r\n"
    return(data)
...
class iosetting(object):
    ...
    def Load_setting(self):
        ...
        if(len(setting)>=51):
            ...
            self.calibration_status=int(setting[1])
...
def setup():
    ...
    GPIO.add_event_detect(INP1, GPIO.RISING, callback=inputEventHandler)
    GPIO.add_event_detect(INP2, GPIO.RISING, callback=inputEventHandler)
                    ...
                    if(data[fieldstr]!=None):
                        ...
                        if(totalmin1>totalmin2-5 and totalmin1<totalmin2+5):
                            ...
                            if(usbrtu.status[i]==0):
                                usbrtu.status[i]=1
                            Calibration.realvalue[i]=round((usbrtu.readdata[i]*Calibration.gain[i]+Calibration.offset[i]),1)   
                            
                            #print ("read thing=",(Calibration.realvalue[i]))
                        else:
                            usbrtu.readdata[i]=0
                            #usbrtu.readdata[i]=float(data[fieldstr])
                            Calibration.realvalue[i]=round((usbrtu.readdata[i]*Calibration.gain[i]+Calibration.offset[i]),1) 
...
def updateFieldThingspeak():
    ...
    if(len(Netsetting.portserver)):
        port=int(Netsetting.portserver)       
    if(port==8888):
        if(len(Netsetting.ipserver)>=16):
            baseURL1 = 'https://api.thingspeak.com/update?api_key=%s' % Netsetting.ipserver
            fieldthing1=''
            fieldthing2=''
            for i in range(Set.maxchannel):
                if(i<8):
                    fieldthing1=fieldthing1+'&field'+str(i+1)+'='+str(Calibration.realvalue[i])
                elif(i>=8):
                    fieldthing2=fieldthing2+'&field'+str(i+1-8)+'='+str(Calibration.realvalue[i])   
            print(baseURL1+fieldthing1)
             # To Thing cloud 1
            try:
                f = urllib.request.urlopen(baseURL1+fieldthing1, timeout=30)
                resp = f.read().decode('utf8')
                Netsetting.reportserver="Sent Data Cloud 1:"+str(resp)
                print(Netsetting.reportserver)
                GPIO.output(LEDCONNECT,not GPIO.input(LEDCONNECT))
            except Exception as e:
                print("Error thing:"+str(e))
                Netsetting.reportserver="Data Cloud 1 not connect"
                
            # To Thing cloud 2
            if(len(Netsetting.serverftp)>=16 and len(fieldthing2)>0):
                baseURL2 = 'https://api.thingspeak.com/update?api_key=%s' % Netsetting.serverftp
                print(baseURL2+fieldthing2)
                try:
                    f = urllib.request.urlopen(baseURL2+fieldthing2, timeout=30)
                    resp = f.read().decode('utf8')
                    Netsetting.reportserver="Sent Data Cloud 2:"+str(resp)
                    print(Netsetting.reportserver)
                    GPIO.output(LEDCONNECT,not GPIO.input(LEDCONNECT))
                except Exception as e:
                    print("Error thing:"+str(e))
                    Netsetting.reportserver="Data Cloud 2 not connect"
...
def CR6_Request():
    indexchannel=0
    indexchannel_temp=0
    req = urllib.request.Request('http://192.168.1.110/?command=NewestRecord&table=Public')
    try:
        response=urllib.request.urlopen(req,timeout=10)
        #the_page = response.read()
        the_page = str(response.read(), encoding='utf8')
        #print(the_page)
        #field=Mux1_Ch2_Temp&amp;value=24.20328">24.20328</a>
        #field=Mux2_Ch13_Temp&amp;value=NAN">NAN</a>
        #field=Mux1_Ch1&amp;value=8528.658">8528.658</a>
        #field=Mux1_Ch2&amp;value=7857.886">7857.886</a>
        for j in range (1,16):
            stringvalue="field=Mux1_Ch"+str(j)+"&amp;value="
            lenstringvalue=len(stringvalue)
            startstr=the_page.find(stringvalue)
            if(startstr>0):
                for i in range(startstr+lenstringvalue,startstr+lenstringvalue+12):
                    if(the_page[i]=='"'):
                        break
                valuestr=str(the_page[startstr+lenstringvalue:i])
                #print(valuestr)
                if(valuestr!='NAN' and len(valuestr)>0):
                    valuefloat=float(valuestr)
                    print(str(stringvalue)+str(valuefloat))
                    indexchannel=j-1
                    if(Set.functionchannel[indexchannel]==32 and indexchannel<8):
                        usbrtu.readdata[indexchannel]=valuefloat
                        Calibration.realvalue[indexchannel]=round((usbrtu.readdata[indexchannel]*Calibration.gain[indexchannel]+Calibration.offset[indexchannel]),2)
                        if(usbrtu.status[indexchannel]==0 or IOsetting.alarm==0):
                            usbrtu.status[indexchannel]=1

                elif(valuestr=='NAN'):
                    valuefloat=0
                    print(str(stringvalue)+str(valuefloat))
                    indexchannel=j-1
                    if(Set.functionchannel[indexchannel]==32 and indexchannel<16):
                        usbrtu.readdata[indexchannel]=valuefloat
                        Calibration.realvalue[indexchannel]=round((usbrtu.readdata[indexchannel]*Calibration.gain[indexchannel]+Calibration.offset[indexchannel]),2)
                        usbrtu.status[indexchannel]=0           
        '''for j in range (1,16):
            stringvalue="field=Mux1_Ch"+str(j)+"_Temp&amp;value="
            lenstringvalue=len(stringvalue)
            startstr=the_page.find(stringvalue)
            if(startstr>0):
                for i in range(startstr+lenstringvalue,startstr+lenstringvalue+12):
                ...
                if(valuestr!='NAN' and len(valuestr)>0):
                    valuefloat=float(valuestr)
                    print(str(stringvalue)+str(valuefloat))
                    indexchannel_temp=indexchannel+j-1
                    if(Set.functionchannel[indexchannel_temp]==32 and indexchannel_temp<8):
                        usbrtu.readdata[indexchannel_temp]=valuefloat
                        Calibration.realvalue[indexchannel_temp]=round((usbrtu.readdata[indexchannel_temp]*Calibration.gain[indexchannel_temp]+Calibration.offset[indexchannel_temp]),2)
                        if(usbrtu.status[indexchannel_temp]==0 or IOsetting.alarm==0):
                            usbrtu.status[indexchannel_temp]=1
                            
        #field=BattV&amp;value=12.56702">12.56702</a>
        stringvalue="field=BattV&amp;value="
        lenstringvalue=len(stringvalue)
        startstr=the_page.find(stringvalue)
        if(startstr>0):
            for i in range(startstr+lenstringvalue,startstr+lenstringvalue+12):
            ...
            valuestr=str(the_page[startstr+lenstringvalue:i])
            if(valuestr!='NAN' and len(valuestr)>0):
                indexchannel_temp=indexchannel_temp+1
                valuefloat=float(valuestr)
                if(Set.functionchannel[indexchannel_temp]==32 and indexchannel_temp<8):
                    usbrtu.readdata[indexchannel_temp]=valuefloat
                    Calibration.realvalue[indexchannel_temp]=round((usbrtu.readdata[indexchannel_temp]*Calibration.gain[indexchannel_temp]+Calibration.offset[indexchannel_temp]),2)
                    if(usbrtu.status[indexchannel_temp]==0 or IOsetting.alarm==0):
                        usbrtu.status[indexchannel_temp]=1'''
        usbrtu.reportmodbus=str(stringvalue)+str(valuefloat)
        print(usbrtu.reportmodbus)              
    except Exception as e:
        usbrtu.reportmodbus="Not connect Campbell:"+str(e)
        for i in range (Set.maxchannel):
            if(Set.functionchannel[i]==32):
                usbrtu.status[i]=0
                usbrtu.readdata[i]=0
                Calibration.realvalue[i]=round((usbrtu.readdata[i]*Calibration.gain[i]+Calibration.offset[i]),2)
#-------------------------------------------------------
class MAfilter:
...
class raingause(object):
    def __init__(self):
        ...
        with sqlite3.connect(dbname) as conn:
            ...
            for row in curs.fetchmany(Set.maxchannel):
                #data=data+str(row[1])+","+str(row[2])+","+str(row[3]+1)+","+ Set.namechannel[row[3]] + "," + str(row[4])+","+Set.unitreg[row[3]]+","+str(row[5])+"\r\n"
                '''if(row[3]==0):
                    IOsetting.pulseinput[0]=row[4]/Calibration.gain[0]
                    self.rainhour=row[4]'''
                if(row[3]==1):
                    self.rainday=row[4]
                elif(row[3]==2):
    ...
    def calculator(self,rainnow):
        ...
        if(self.rainhour>self.rainhourlast):
            ...
            usbrtu.reportmodbus="Continuous Rainfall mm="+str(self.raincontinuebuff)+'+='+str(self.rainhour)+'-'+str(self.rainhourlast)
            print(usbrtu.reportmodbus)
...
def loop():
    ...
    global countsec,countmin,Beep, inputstatus, outputstatus,modeoutputstatus,lastsecond,lastminute,lasthour,datetimealarms,looprunning,indextask,connect485
    global indexvalue,counthour,indexmodbus
    global newdatahistory,timesecnow,ads0_analog,ads1_analog
    countsecnow=0
    try:
        realvalue=0
        data=0
        ...
        if(now.second!=lastsecond):
            ...
            if(countsec>30):    #20 giay kiem tra tin nhan toi 1 lan
                ...
                datachannel=''
                for i in range(Set.maxchannel):
                    datachannel=datachannel+str(Calibration.realvalue[i])+' '
                    ...
                    elif(Set.functionchannel[i]==6 and Set.typereg[i]==4  and Set.startreg[i]==2 and usbrtu.writefc6[i]==0):
                        usbrtu.writefc6[i]=1
                        usbrtu.writedatafc6[i]=datachannel
                        print("Writedatafc6 All data channel: "+usbrtu.writedatafc6[i])
                        datachannel=''
        ...
        if(now.minute!=lastminute):
            ...
            if(countmin >= Set.tupload):
                ...
                '''if(len(Netsetting.serverftp)>0 and len(Netsetting.userftp)>0 and len(Netsetting.passftp)>0 and len(Netsetting.filenameftp)):
                    ...
                    upload_content=""
                    for i in range(Set.maxchannel):
                        if(IOsetting.calibration_status==1):
                            statussensor=1
                        elif(usbrtu.status[i]>=1):
                            statussensor=0
                        elif(usbrtu.status[i]==0):
                            statussensor=2
                        upload_content=upload_content+str(Set.namechannel[i])+"\t"+str(Calibration.realvalue[i])+"\t"+str(Set.unitreg[i])+"\t"+\
                                        strftime("%Y%m%d%H%M%S",localtime())+"\t"+str(statussensor)+"\r\n"
            ...
            if(Netsetting.sendtoemail==1):
                ...
                Netsetting.mailto2=""
                try:
                    MAIL(Netsetting.settingtomail,"NetSet from ECA-GPIs6.6DA: "+Netsetting.hostname)
                except Exception as e:
        ...
        if(data==0 and IOsetting.alarm and not q.empty()):
            ...
            try:
                MAIL(Netsetting.hostname+alarms+"\n"+datetimealarms+"\n"+str(Netsetting.ip)+":8880\nhttp://ecapro.com.vn/","Email alarm from ECA-GPIs6.6DA device: "+Netsetting.hostname)
        ...
        if(now.hour!=lasthour):
            ...
            if(IOsetting.tinfor>0):
                ...
                if(counthour>=IOsetting.tinfor):
                    ...
                    try:
                        MAIL(textsms,"Email Infor from ECA-GPIs6.6DA device: "+Netsetting.hostname)
                        Netsetting.reportserver="Sent Mail"   #Khac phuc loi khi nhan tin het tien.
                    except Exception as e:
                        Netsetting.reportserver="Error Mail:"+str(e)   #Khac phuc loi khi nhan tin het tien.
        ...
        # tinh luu uong tong
        if(Set.functionchannel[indexmodbus]==19):
            timeseclast=time.time()-timesecnow
            if(timesecnow>0 and timeseclast>0 and timeseclast<360):
                usbrtu.readdata[indexmodbus]+=Calibration.realvalue[Set.addchannel[indexmodbus]-1]*timeseclast/3600
                ...
                usbrtu.reportmodbus=str(Set.addchannel[indexmodbus])+" Total Flow:"+str(round(usbrtu.readdata[indexmodbus],2))+" Sec:"+str(round(timeseclast,2))
                print(usbrtu.reportmodbus)
                if(usbrtu.status[indexmodbus]==0 or IOsetting.alarm==0):
                    usbrtu.status[indexmodbus]=1
            timesecnow=time.time()
            
        elif(Set.functionchannel[indexmodbus]==23 and indexmodbus<2):
            usbrtu.readdata[indexmodbus]=IOsetting.pulseinput[indexmodbus]
            Calibration.realvalue[indexmodbus]=round((usbrtu.readdata[indexmodbus]*Calibration.gain[indexmodbus]+Calibration.offset[indexmodbus]),2)
            usbrtu.reportmodbus="Rainfall in the hour mm/h="+str(usbrtu.readdata[indexmodbus])
            print(usbrtu.reportmodbus)
            #time.sleep(Set.timeout)
            # tinh toan luong mua mm
            rainsensor.calculator(Calibration.realvalue[indexmodbus])
            time.sleep(Set.timeout)
            if(usbrtu.status[indexmodbus]==0 or IOsetting.alarm==0):
                usbrtu.status[indexmodbus]=1

        elif(Set.functionchannel[indexmodbus]==24 and indexmodbus==1):
            usbrtu.readdata[indexmodbus]=rainsensor.rainday
            Calibration.realvalue[indexmodbus]=round((usbrtu.readdata[indexmodbus]*Calibration.gain[indexmodbus]+Calibration.offset[indexmodbus]),2)
            usbrtu.reportmodbus="Rainfall during the day (mm/24h)="+str(Calibration.realvalue[indexmodbus])
            #print(usbrtu.reportmodbus)
            time.sleep(Set.timeout)
            if(usbrtu.status[indexmodbus]==0 or IOsetting.alarm==0):
                usbrtu.status[indexmodbus]=1
                
        elif(Set.functionchannel[indexmodbus]==25 and indexmodbus==2):
            usbrtu.readdata[indexmodbus]=rainsensor.rainmaxhourinday
            Calibration.realvalue[indexmodbus]=round((usbrtu.readdata[indexmodbus]*Calibration.gain[indexmodbus]+Calibration.offset[indexmodbus]),2)
            usbrtu.reportmodbus="Maximum rainfall (mm/h)="+str(Calibration.realvalue[indexmodbus])
            #print(usbrtu.reportmodbus)
            time.sleep(Set.timeout)
            if(usbrtu.status[indexmodbus]==0 or IOsetting.alarm==0):
                usbrtu.status[indexmodbus]=1
                
        elif(Set.functionchannel[indexmodbus]==26 and indexmodbus==3):
            usbrtu.readdata[indexmodbus]=rainsensor.timerainmaxhourinday
            Calibration.realvalue[indexmodbus]=round((usbrtu.readdata[indexmodbus]*Calibration.gain[indexmodbus]+Calibration.offset[indexmodbus]),2)
            usbrtu.reportmodbus="Maximum rainfall (hour in day)="+str(Calibration.realvalue[indexmodbus])
            #print(usbrtu.reportmodbus)
            time.sleep(Set.timeout)
            if(usbrtu.status[indexmodbus]==0 or IOsetting.alarm==0):
                usbrtu.status[indexmodbus]=1
                
        elif(Set.functionchannel[indexmodbus]==27 and indexmodbus==4):
            usbrtu.readdata[indexmodbus]=rainsensor.rainmaxdayinyear
            Calibration.realvalue[indexmodbus]=round((usbrtu.readdata[indexmodbus]*Calibration.gain[indexmodbus]+Calibration.offset[indexmodbus]),2)
            usbrtu.reportmodbus="Maximum rainfall by day (mm/day)="+str(Calibration.realvalue[indexmodbus])
            #print(usbrtu.reportmodbus)
            time.sleep(Set.timeout)
            if(usbrtu.status[indexmodbus]==0 or IOsetting.alarm==0):
                usbrtu.status[indexmodbus]=1 

        elif(Set.functionchannel[indexmodbus]==28 and indexmodbus==5):
            usbrtu.readdata[indexmodbus]=rainsensor.timerainmaxdayinyear
            Calibration.realvalue[indexmodbus]=usbrtu.readdata[indexmodbus]
            usbrtu.reportmodbus="Maximum rainfall day (yymmdd)="+str(Calibration.realvalue[indexmodbus])
            #print(usbrtu.reportmodbus)
            time.sleep(Set.timeout)
            if(usbrtu.status[indexmodbus]==0 or IOsetting.alarm==0):
                usbrtu.status[indexmodbus]=1
                
        elif(Set.functionchannel[indexmodbus]==29 and indexmodbus==6):
            usbrtu.readdata[indexmodbus]=rainsensor.raincontinue
            Calibration.realvalue[indexmodbus]=round((usbrtu.readdata[indexmodbus]*Calibration.gain[indexmodbus]+Calibration.offset[indexmodbus]),2)
            time.sleep(Set.timeout)
            if(usbrtu.status[indexmodbus]==0 or IOsetting.alarm==0):
                usbrtu.status[indexmodbus]=1
                
        elif(Set.functionchannel[indexmodbus]==30 and indexmodbus==7):
            usbrtu.readdata[indexmodbus]=rainsensor.raincontinuedura
            Calibration.realvalue[indexmodbus]=round((usbrtu.readdata[indexmodbus]*Calibration.gain[indexmodbus]+Calibration.offset[indexmodbus]),2)
            usbrtu.reportmodbus="Long duration of rain(hour)="+str(Calibration.realvalue[indexmodbus])
            print(usbrtu.reportmodbus)
            time.sleep(Set.timeout)
            if(usbrtu.status[indexmodbus]==0 or IOsetting.alarm==0):
                usbrtu.status[indexmodbus]=1
                
        elif(Set.functionchannel[indexmodbus]==31 and indexmodbus==8):
            usbrtu.readdata[indexmodbus]=rainsensor.raincontinuetimeend
            Calibration.realvalue[indexmodbus]=usbrtu.readdata[indexmodbus]
            usbrtu.reportmodbus="Rain time end (yymmddhh)="+str(Calibration.realvalue[indexmodbus])
            print(usbrtu.reportmodbus)
            time.sleep(Set.timeout)
            if(usbrtu.status[indexmodbus]==0 or IOsetting.alarm==0):
                usbrtu.status[indexmodbus]=1  
        
        elif(usbrtu.connect==True and Set.functionchannel[indexmodbus]<7):
            indextask=4.3
            try:
                #for i in range(Set.maxchannel):
        ...
        # Doc gia tri do ADC0
        elif(ads0_analog==True and Set.functionchannel[indexmodbus]==7 and indexmodbus<4):
            indextask=4.4
            ...
            if(Set.startreg[indexmodbus]==0):
                usbrtu.readdata[indexmodbus]=ads0.analogRead(indexads)
                usbrtu.reportmodbus="ADC:"+str(Set.addchannel[indexmodbus])+" analog="+str(usbrtu.readdata[indexmodbus])
            else:
                usbrtu.readdata[indexmodbus]=ads0.analogReadVolt(indexads)
        ...
        # Doc gia tri do ADC1
        elif(ads1_analog==True and Set.functionchannel[indexmodbus]==7 and indexmodbus>=4 and indexmodbus<8):
            indextask=4.5
            indexads=indexmodbus-3
            if(indexads>=4):
                indexads=0

            if(Set.startreg[indexmodbus]==0):
                usbrtu.readdata[indexmodbus]=ads1.analogRead(indexads)
                usbrtu.reportmodbus="ADC:"+str(Set.addchannel[indexmodbus])+" analog="+str(usbrtu.readdata[indexmodbus])
            else:
                usbrtu.readdata[indexmodbus]=ads1.analogReadVolt(indexads)
                usbrtu.reportmodbus="ADC:"+str(Set.addchannel[indexmodbus])+" Volt="+str(usbrtu.readdata[indexmodbus])
            print(usbrtu.reportmodbus)
            time.sleep(Set.timeout)
            if(usbrtu.status[indexmodbus]==0 or IOsetting.alarm==0):
...
'''def readmodbusrtu():
    global countsec,countmin,Beep, inputstatus, outputstatus,modeoutputstatus,lastsecond,lastminute,lasthour,datetimealarms,looprunning,indextask,connect485
    global indexvalue,counthour,indexmodbus
    global newdatahistory,ads0_analog,ads1_analog
    timesecnow=0
    timeseclast=0
    ...
    indexmodbus=0
    while True:
        # Phan tinh tong luu luong=m3/h=m3/60x60=3600 sec
        if(Set.functionchannel[indexmodbus]==19):
            timeseclast=time.time()-timesecnow
            if(timesecnow>0 and timeseclast>0):
                usbrtu.readdata[indexmodbus]+=Calibration.realvalue[Set.addchannel[indexmodbus]-1]*timeseclast/3600
                Calibration.realvalue[indexmodbus]=round((usbrtu.readdata[indexmodbus]*Calibration.gain[indexmodbus]+Calibration.offset[indexmodbus]),2)
                usbrtu.reportmodbus=str(Set.addchannel[indexmodbus])+" Total Flow:"+str(round(usbrtu.readdata[indexmodbus],2))+" Sec:"+str(round(timeseclast,2))
                print(usbrtu.reportmodbus)
        ...
        # Doc gia tri do ADC0
        elif(ads0_analog==True and Set.functionchannel[indexmodbus]==7 and Set.addchannel[indexmodbus]<4):
            indextask=4.4
            indexads=indexmodbus+1
            if(indexads>=4):
                indexads=0
            if(Set.startreg[indexmodbus]==0):
                usbrtu.readdata[indexmodbus]=ads0.analogRead(indexmodbus)
                usbrtu.reportmodbus="ADC:"+str(indexmodbus)+" analog="+str(usbrtu.readdata[indexmodbus])
            else:
                usbrtu.readdata[indexmodbus]=ads0.analogReadVolt(indexmodbus)
                usbrtu.reportmodbus="ADC:"+str(indexmodbus)+" Volt="+str(usbrtu.readdata[indexmodbus])
            print(usbrtu.reportmodbus)
            time.sleep(Set.timeout)
            if(usbrtu.status[indexmodbus]==0 or IOsetting.alarm==0):
                usbrtu.status[indexmodbus]=1
        ...
        # Doc gia tri do ADC1
            ...
            if(Set.startreg[indexmodbus]==0):
                usbrtu.readdata[indexmodbus]=ads1.analogRead(indexmodbus-4)
                usbrtu.reportmodbus="ADC:"+str(indexmodbus)+" analog="+str(usbrtu.readdata[indexmodbus])
            else:
                usbrtu.readdata[indexmodbus]=ads1.analogReadVolt(indexmodbus-4)
                usbrtu.reportmodbus="ADC:"+str(indexmodbus)+" Volt="+str(usbrtu.readdata[indexmodbus])
            print(usbrtu.reportmodbus)
            time.sleep(Set.timeout)
            if(usbrtu.status[indexmodbus]==0 or IOsetting.alarm==0):
                    ...
                    with regs_lock:
                        ...
                        if(ModbusTCP.selectchannel[i]>0 and ModbusTCP.numberreg[i]>0):
                            ...
                            for j in range(ModbusTCP.selectchannel[i]-1,ModbusTCP.selectchannel[i]+ModbusTCP.numberreg[i]-1):
                                ...
                                Calibration.realvalue[j]=round((usbrtu.readdata[j]*Calibration.gain[j]+Calibration.offset[j]),1)    
                                if(usbrtu.status[j]==0):
                                    usbrtu.status[j]=1
                                m=m+1
...
def blink():
    ...
    while True:
        ...
        if(looprunning==1 and now.minute==0): #60 phut kiem tra 1 lan
            ...
            try:
                MAIL(textsms,"Email Error from ECA-GPIs6.6DA device: "+Netsetting.hostname)
                #Netsetting.reportserver="Sent Mail"   #Khac phuc loi khi nhan tin het tien.
            except:
