...
global datedatachart,daydatachart,lastchannel,ads0_analog,ads1_analog,indexmodbus,timesecnow,indexthingspeak
indexthingspeak=0
timesecnow=0
indexmodbus=0
inputstatus=0
...
def GetLastEntryFieldThingspeak(indexchannel):
    channelid=""
    channelapi=""
    if(indexchannel==0):
        if(len(Netsetting.portserver)>4 and len(Netsetting.ipserver)>=16):
            channelid=Netsetting.portserver
            channelapi=Netsetting.ipserver
        
    elif(indexchannel==1):
        if(len(Netsetting.serverftp)>=16 and len(Netsetting.filenameftp)>=4):
            channelid=Netsetting.filenameftp
            channelapi=Netsetting.serverftp
             
    elif(indexchannel==2):        
        if(len(Netsetting.pathftp)>=16 and len(Netsetting.userftp)>=4):
            channelid=Netsetting.userftp
            channelapi=Netsetting.pathftp

    elif(indexchannel==3):        
        if(len(Netsetting.serverftp2)>=16 and len(Netsetting.pathftp2)>=4):
            channelid=Netsetting.pathftp2
            channelapi=Netsetting.serverftp2
            
    if(len(channelid)>=4 and len(channelapi)>=16):     
        with urllib.request.urlopen("http://api.thingspeak.com/channels/%s/feeds/last.json?api_key=%s" % (channelid,channelapi)) as conn:
            response = conn.read().decode('utf8')
            print ("http status code=%s" % (conn.getcode()))
            data=json.loads(response)
            print (str(data)+str(len(data))) #32.5 2017-08-22T07:44:36Z
            if(len(data)<3):
                return
            Netsetting.reportserver=str(indexchannel+1)+" Cloud:"+str(channelid)+" status:"+str(conn.getcode())+" entry:"+str(data['entry_id'])
            GPIO.output(LEDCONNECT,not GPIO.input(LEDCONNECT))
            if(indexchannel==0):
                maxfield=4
            elif(indexchannel==1):
            ...
            for i in range(0,maxfield):
                if(indexchannel==0):
                    ...
                    indexthingchannel=4+i
                elif(indexchannel==2):
                    indexthingchannel=4+8+i
                elif(indexchannel==3):
                    indexthingchannel=4+8+8+i
                #{'field2': '0.0', 'field3': '0.0', 'created_at': '2017-11-23T03:40:05Z', 'field1': '0.0', 'field4': '0.0', 'entry_id': 1376}
                if(Set.functionchannel[indexthingchannel]==10):
                    fieldstr="field"+str(i+1)
                    if(fieldstr in data and data[fieldstr]!=None):     
                        ...
                        if(totalmin1>totalmin2-5 and totalmin1<totalmin2+5):
                            if(len(data[fieldstr])==0):
                                usbrtu.readdata[indexthingchannel]=0
                                usbrtu.status[indexthingchannel]=0
                            else:
                                usbrtu.readdata[indexthingchannel]=float(data[fieldstr])
                                if(usbrtu.status[indexthingchannel]==0):
                                    usbrtu.status[indexthingchannel]=1
                            Calibration.realvalue[indexthingchannel]=round((usbrtu.readdata[indexthingchannel]*Calibration.gain[indexthingchannel]+Calibration.offset[indexthingchannel]),1)   
...
def loop():
    ...
    global newdatahistory,timesecnow,ads0_analog,ads1_analog,indexthingspeak
    countsecnow=0
    try:
        realvalue=0
        data=0
        ...
        if(now.second!=lastsecond):
            ...
            if(countsec>10):    #20 giay kiem tra tin nhan toi 1 lan
                countsec=0
                #Get thingspeak
                GetLastEntryFieldThingspeak(indexthingspeak)
                indexthingspeak+=1
                if(indexthingspeak>=4):     # Doc 4 kenh thingspeak
                    indexthingspeak=0
                ...
                if(counthour>=IOsetting.tinfor):
                    ...
                    for i in range(Set.maxchannel):
                        ...
                        if(usbrtu.status[i]==0):
                            Channelstatus="\tNot Connect"
                        if(usbrtu.status[i]==1):
                            Channelstatus="\tConnected"
                        if(usbrtu.status[i]==2):
                            Channelstatus="\tLow Alarm"
                        if(usbrtu.status[i]==3):
                            Channelstatus="\tHigh Alarm"
                        textsms=textsms+str(Set.namechannel[i])+"\t"+str(Calibration.realvalue[i])+"\t"+str(Set.unitreg[i])+str(Channelstatus)+"\n"
        ...
        # Do luong mua kenh IN1
        elif(Set.functionchannel[indexmodbus]==23):
            usbrtu.readdata[indexmodbus]=IOsetting.pulseinput[0]
            Calibration.realvalue[indexmodbus]=round((usbrtu.readdata[indexmodbus]*0.2),2)
            ...
            if(usbrtu.status[indexmodbus]==0 or IOsetting.alarm==0):
                usbrtu.status[indexmodbus]=1
        # Do luong mua thao ngay
        elif(Set.functionchannel[indexmodbus]==24):
        ...
        # Doc gia tri do ADC0
            ...
            if(Set.startreg[indexmodbus]==0):
                adcread=ads0.analogRead(indexads)
                usbrtu.readdata[indexmodbus]=Fir.minmaxmedium(adcread,indexmodbus)
                if(usbrtu.status[indexmodbus]==0 or IOsetting.alarm==0):
                    usbrtu.status[indexmodbus]=1
                usbrtu.reportmodbus="ADC:"+str(Set.addchannel[indexmodbus])+" analog="+str(usbrtu.readdata[indexmodbus])
            else:
                adcvolt=ads0.analogReadVolt(indexads)   #1mA*200ohm=200mV
                usbrtu.readdata[indexmodbus]=Fir.minmaxmedium(adcvolt,indexmodbus)
                if(usbrtu.status[indexmodbus]==0 or IOsetting.alarm==0):
                    usbrtu.status[indexmodbus]=1
        ...
        # Doc gia tri do ADC1
            ...
            if(Set.startreg[indexmodbus]==0):
                adcread=ads1.analogRead(indexads)
                usbrtu.readdata[indexmodbus]=Fir.minmaxmedium(adcread,indexmodbus)
                if(usbrtu.status[indexmodbus]==0 or IOsetting.alarm==0):
                    usbrtu.status[indexmodbus]=1
                usbrtu.reportmodbus="ADC:"+str(Set.addchannel[indexmodbus])+" analog="+str(usbrtu.readdata[indexmodbus])
            else:
                adcvolt=ads1.analogReadVolt(indexads)   #1mA*200ohm=200mV
                usbrtu.readdata[indexmodbus]=Fir.minmaxmedium(adcvolt,indexmodbus)
                if(usbrtu.status[indexmodbus]==0 or IOsetting.alarm==0):
                    usbrtu.status[indexmodbus]=1
                usbrtu.reportmodbus="ADC:"+str(Set.addchannel[indexmodbus])+" Volt="+str(usbrtu.readdata[indexmodbus])
