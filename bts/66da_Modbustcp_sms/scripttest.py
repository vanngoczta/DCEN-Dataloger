...
def load_history_data_day_attachment(days):
    i=0
    data=""
    with sqlite3.connect(dbname,isolation_level=None) as conn:
        curs=conn.cursor()
...
def updateStatusThingspeak(status):
    ...
    if(port==8888 and len(Netsetting.ipserver)>=16):
        ...
        print(params )
        try:
            f=urllib.request.urlopen("https://api.thingspeak.com/update", data=params, timeout=10)
...
def updateFieldThingspeak():
    ...
    if(port==8888 and len(Netsetting.ipserver)>=16):
        ...
        try:
            f = urllib.request.urlopen(baseURL+fieldthing)
            resp = f.read()
            print(resp)
        except Exception as e:
            print("Error thing:"+e)
...
def loop():
    ...
    if(now.second!=lastsecond):
        ...
        if(countsec>40):    #20 giay kiem tra tin nhan toi 1 lan
            ...
            if(IOsetting.alarm==0 and data=="ARMING"):
                updateStatusThingspeak("DISARM")
            if(IOsetting.alarm==1 and data=="DISARM"):
                updateStatusThingspeak("ARMING")
