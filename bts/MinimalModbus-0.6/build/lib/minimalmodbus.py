...
"""Default value for the parity. See the pySerial module for documentation. Defaults to serial.PARITY_NONE, serial.PARITY_EVEN"""
...
class Instrument():
    ...
    def write_bit(self, registeraddress, value, functioncode=5):
        ...
        return self._genericCommand(functioncode, registeraddress, value)
    ...
    def read_float(self, registeraddress, functioncode=3, numberOfRegisters=2, formatcodenumber=0):
        ...
        Args:
            ...
            * formatcodenumber int: 0 - float, 1 - floatDCBA , 2 - floatCDAB
        ...
        if(formatcodenumber==1):
            return self._genericCommand(functioncode, registeraddress, numberOfRegisters=numberOfRegisters, payloadformat='floatDCBA')
        elif(formatcodenumber==2):
            return self._genericCommand(functioncode, registeraddress, numberOfRegisters=numberOfRegisters, payloadformat='floatCDAB')
        else:
    ...
    def _genericCommand(self, functioncode, registeraddress, value=None, \
        ...
        PAYLOADFORMAT_FLOAT_DCBA     = 'floatDCBA'
        PAYLOADFORMAT_FLOAT_CDAB     = 'floatCDAB'
        PAYLOADFORMAT_STRING    = 'string'
        PAYLOADFORMAT_REGISTER  = 'register'
        PAYLOADFORMAT_REGISTERS = 'registers'

        ALL_PAYLOADFORMATS = [PAYLOADFORMAT_LONG, PAYLOADFORMAT_FLOAT, PAYLOADFORMAT_FLOAT_DCBA, PAYLOADFORMAT_FLOAT_CDAB, \
        ...
        # ECAPRO update 02/11/2016
        if functioncode in [5, 6]:
            registerdata = payloadFromSlave[NUMBER_OF_BYTES_BEFORE_REGISTERDATA:]
            return len(registerdata)
        ...
        if functioncode in [3, 4]:
            ...
            if payloadformat == PAYLOADFORMAT_STRING:
                ...
                return _bytestringToFloat(registerdata, numberOfRegisters,0)
            elif payloadformat == PAYLOADFORMAT_FLOAT_DCBA:
                return _bytestringToFloat(registerdata, numberOfRegisters,1)
            elif payloadformat == PAYLOADFORMAT_FLOAT_CDAB:
                return _bytestringToFloat(registerdata, numberOfRegisters,2)
...
def _floatToBytestring(value, numberOfRegisters=2):
    ...
    formatcode = '<'  # Big-endian sua lai > thanh <
...
def _bytestringToFloat(bytestring, numberOfRegisters=2, formatcodenunber=0):
    ...
    bytestringout=bytestring
    if(formatcodenunber==0):
        formatcode = '>'  # Big-endian      > ABCD
    elif(formatcodenunber==1):
        formatcode = '<'  # little endian   < DCBA
    elif(formatcodenunber==2 and numberOfRegisters == 2):
        formatcode = '>'  # Big-endian      > CDAB
        bytestringout=bytestring[2:3]+bytestring[3:4]+bytestring[0:1]+bytestring[1:2]
    ...
    return _unpack(formatcode, bytestringout)
