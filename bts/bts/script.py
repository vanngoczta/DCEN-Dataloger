...
def UpdateStatus():
    ...
    data=data+Netsetting.reportserver[:60]+","+ IOsetting.reporthmi+". "+Rea.reportmodbus[:50]+"\r\n"
    return(data)
...
class netsetting(object):
    ...
    def Load_setting(self):     
        ...
        if(len(setting)>=36):
            ...
            # Su dung 2 kenh thingspeak
            # 08/09/18
            leng=self.ipserver.find('|')
            if(leng>0):
                self.thingAPIKEY[0]=self.ipserver[0:leng]
                self.thingAPIKEY[1]=self.ipserver[leng+1:]
            else:
                self.thingAPIKEY[0]=self.ipserver
                self.thingAPIKEY[1]=""
            print(self.thingAPIKEY)
            leng=self.portserver.find('|')
            if(leng>0):
                self.thingID[0]=self.portserver[0:leng]
                self.thingID[1]=self.portserver[leng+1:]
            else:
                self.thingID[0]=self.portserver
                self.thingID[1]=""
            print(self.thingID)
            # 15/05/18
...
#--------------------------------------------------------
#serial2 = serial.Serial(port='/dev/ttyUSB1',baudrate = 9600,parity=serial.PARITY_NONE,stopbits=serial.STOPBITS_ONE,bytesize=serial.EIGHTBITS,timeout=0.5)
...
def setup():
    ...
    if(IOsetting.test_ds1307>0):
        Netsetting.reportserver="Error DS1307 RTC"
    # Setup GPIOs
    GPIO.setwarnings(False) 
    GPIO.setup(LEDRUN, GPIO.OUT) # sets i to output and 0V, off
    ...
    try:
        ...
        Netsetting.reportserver="Error:"+str(e)
#-----------update 070417---------------
...
def updateStatusThingspeak(status):
    port=0
    if(len(Netsetting.thingID[0])):
        port=int(Netsetting.thingID[0])        
    if(port==8888 and len(Netsetting.thingAPIKEY[0])>=16):
        try:
            params = urllib.parse.urlencode({'key': Netsetting.thingAPIKEY[0],'status':status}).encode("utf-8")
            print(params )
...
def GetLastEntryFieldThingspeak():
    if(Netsetting.thingIndex==0):
        Netsetting.thingIndex=1
    else:
        Netsetting.thingIndex=0
    if(len(Netsetting.thingID[Netsetting.thingIndex])>4 and len(Netsetting.thingAPIKEY[Netsetting.thingIndex])>=16):
        with urllib.request.urlopen("http://api.thingspeak.com/channels/%s/feeds/last.json?api_key=%s" % (Netsetting.thingID[Netsetting.thingIndex],Netsetting.thingAPIKEY[Netsetting.thingIndex])) as conn:
            ...
            GPIO.output(LEDCONNECT,not GPIO.input(LEDCONNECT))
            for i in range(16):
                if(Set.functionchannel[i]==10 and Netsetting.thingIndex==Set.startreg[i]):
                    m=i
                    fieldstr="field"+str(Set.addchannel[m])
                    if(data[fieldstr]!=None):
                        ...
                        totalmin2=now.hour*60+now.minute'''
                         # tinh thoi gian update thingspeak
                        sectimething=datetime.datetime.strptime(data['created_at'],"%Y-%m-%dT%H:%M:%SZ")
                        mintimething=int((time.mktime(sectimething.timetuple())+25200)/60)
                        # tinh thoi gian hien tai cua thiet bi
                        sectimenow = datetime.datetime.strptime(datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'), '%Y-%m-%d %H:%M:%S')
                        mintimenow=int(time.mktime(sectimenow.timetuple())/60)
                        
                        print (fieldstr,data[fieldstr],mintimething,mintimenow,mintimenow-mintimething)
                        Netsetting.reportserver=fieldstr+": "+data[fieldstr]+" min: "+str(mintimenow-mintimething)
                        if(mintimething>mintimenow-10 and mintimething<mintimenow+10):
                            Rea.Val[m]=float(data[fieldstr])
                            if(Rea.status[m]==0):
                                Rea.status[m]=1
                            CaL.ReV[m]=round((Rea.Val[m]*CaL.gain[m]+CaL.offset[m]),1)   
                            
                            #print ("read thing=",(CaL.ReV[m]))
                        else:
                            Rea.Val[m]=0
                            #Rea.Val[m]=float(data[fieldstr])
                            CaL.ReV[m]=round((Rea.Val[m]*CaL.gain[m]+CaL.offset[m]),1) 
...
def updateFieldThingspeak():
    ...
    if(port==8888 and len(Netsetting.thingAPIKEY[0])>=16):
        try:
            baseURL = 'https://api.thingspeak.com/update?api_key=%s' % Netsetting.thingAPIKEY[0]
            fieldthing=''
            for i in range(Set.maxchannel):
...
try:
    ...
    Netsetting.reportserver="Database empty:"+str(e)
...
def loop():
    ...
    try:
        ...
        if(now.minute!=lastminute):
            ...
            # TRUY CAP WEB ECAPRO
            '''try:
                url ="https://www.youtube.com/watch?v=_1C46s1sq6Q"
            ...
            if(now.minute % Set.tupload==0):
                ...
                if(len(Netsetting.serverftp)>0 and len(Netsetting.userftp)>0 and len(Netsetting.passftp)>0 and len(Netsetting.filenameftp)):
                    #TenTinh_TenCoso_TenTram_Thoigian.txt
                    file_name_ftp=Netsetting.filenameftp+'_'+strftime("%Y%m%d%H%M",localtime())+"00.txt"
                    ...
                    for i in range(Set.maxchannel):
                        ...
                        # Sensor good
                            ...
                            statussensor=0
                        # Sensor Error 
                        elif(Rea.status[i]==0 or Rea.status[i]>1):
                            statussensor=2
                            
                        upload_content=upload_content+str(Set.namechannel[i])+"\t"+str(CaL.ReV[i])+"\t"+str(Set.unitreg[i])+"\t"+\
                                        strftime("%Y%m%d%H%M",localtime())+"00\t0"+str(statussensor)+"\r\n"
            ...
            if(Netsetting.sendtoemail==1):
                ...
                Netsetting.mailto2=""
                if(IOsetting.test_ds1307>0):
                    Netsetting.settingtomail=Netsetting.settingtomail+"\nError DS1307"
                Netsetting.settingtomail=Netsetting.settingtomail+"\n"+Netsetting.reportserver+"\n"+Gsm.reportsms
                Netsetting.settingtomail=Netsetting.settingtomail+"\n"+Rea.reportmodbus+"\nModbus connected:"+str(Set.connect)+"/"+str(Set.maxchannel)
                try:
                    MAIL(Netsetting.settingtomail,"NetSet ECA-GPIs6.6DA: "+Netsetting.hostname+Netsetting.id)
                except Exception as e:
...
class AlarmEvents(object):
    ...
    def Eventtemphumi(self):
        ...
        for i in range(Set.maxchannel):
            # Neu mat ket noi thi khong kiem tra nguong
            if(Rea.status[i]==0):
                continue
            # Gia tri cao
            if(CaL.ReV[i]>float(Set.highset[i])):
                ...
                if(self.Tdelay[i]>10):
                    ...
                    self.Tdelay[i]=0
                    indextask=4.0
                    # Tu dong lay mau khi vuot nguong
                    if (IOsetting.modeoutput[5]==0):
                        GPIO.output(OUT6,1)
                        IOsetting.tonout6buff=IOsetting.tonout6
            # Gia tri thap
...
class ModbusRTU(object):
    ...
    def readmodbus(self,indexaddress,address,functionchannel,startreg,numberreg,typereg):
        ...
        try:
            ...
            #read_float(registeraddress, functioncode=3, numberOfRegisters=2) ABCD
            elif(Set.typereg[indexaddress]==3):                
                self.data = round(self.instrument.read_float(startreg,functionchannel,numberreg,0),2)
            ...
            #read_Float DCBA
            elif(Set.typereg[indexaddress]==7):                         
                self.data = round(self.instrument.read_float(startreg,functionchannel,numberreg,1),2)
                
            #read_Float CDAB
            elif(Set.typereg[indexaddress]==8):                
                self.data = round(self.instrument.read_float(startreg,functionchannel,numberreg,2),2)
...
def polling_thread():
    ...
    while True:
        if(len(ModbusTCP.ipaddress)>0 and len(ModbusTCP.port)>0):
            c = ModbusClient(host=str(ModbusTCP.ipaddress), port=int(ModbusTCP.port),timeout=int(ModbusTCP.timeout))
            print("Connect Modbus TCP"+str(c))
        else:
            print("Not setting Modbus TCP")
            return
        # keep TCP open
        if not c.is_open():
            print("Try Connect Modbus TCP")
            try:
                c.open()
            except Exception as e:
                IOsetting.reporthmi="Modbus TCP not connect! "+str(ModbusTCP.ipaddress)+":"+str(ModbusTCP.port)
                print(IOsetting.reporthmi)
                webiopi.debug(IOsetting.reporthmi)
                for i in range(Set.maxchannel):
                    if(Set.functionchannel[i]==11):     #modbus tcp
                        Rea.status[i]=0
                        Rea.Val[i]=0
                pass
        print("Next Modbus TCP")
        ...
        if c.is_open():
            ...
            for i in range(4):
                print(ModbusTCP.addchannel[i],ModbusTCP.numberreg[i],ModbusTCP.functionchannel[i])
                ...
                if reg_list:
                    with regs_lock:
                        ...
                        if(ModbusTCP.selectchannel[i]>0 and ModbusTCP.numberreg[i]>0):
                            j = ModbusTCP.selectchannel[i]-1
                            # dang float
                            if(ModbusTCP.typereg[i]==3 and ModbusTCP.numberreg[i]==2 and len(regs)>=2):
                                mypack = struct.pack('>HH',regs[1],regs[0])
                                Rea.Val[j] = struct.unpack('>f', mypack)[0]
                                CaL.ReV[j]=round((Rea.Val[j]*CaL.gain[j]+CaL.offset[j]),2)
                            # dang int
                            elif(ModbusTCP.numberreg[i]==1 and len(regs)>=1):
                                Rea.Val[j] =regs[0]
                                CaL.ReV[j]=round((Rea.Val[j]*CaL.gain[j]+CaL.offset[j]),2)    
                            if(Rea.status[j]==0):
                                Rea.status[j]=1        
...
def blink():
    ...
    while True:
        ...
        if (serial.available()):
            ...
            #answer=serial2.read(4)
            #print(str(answer, encoding='latin1'))
        '''if(serial2.readable()):
            readser2=serial2.readline()
            print(readser2)
            print("Length:",str(len(readser2)))
            if(len(readser2)>0):
                #by = bytes(readser2, 'latin1')
                print(str(binascii.hexlify(readser2), encoding='latin1'))'''
        ...
        if(ads0_analog==True):
            try:
                ...
                for m in range(4):
                    valueadc0[m]=0
                    valuevolt0[m]=0
                IOsetting.reporthmi="Error ADC0:"+str(e)
        ...
        if(ads1_analog==True):
            try:
                ...
                for m in range(4):
                    valueadc1[m]=0
                    valuevolt1[m]=0
                IOsetting.reporthmi="Error ADC1:"+str(e)
