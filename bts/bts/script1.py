...
def UpdateStatus():
    ...
    data=data+Netsetting.reportserver[:20]+","+IOsetting.reporthmi[:20]+". "+ usbrtu.reportmodbus[:20]+"\r\n"
    return(data)
...
def loop():
    ...
    try:
        ...
        now = datetime.datetime.now()
        if(now.year>2017 and now.month > 5):
            return
        looprunning=1
        indextask=0.1 
        # Kiem tra theo giay
                ...
                if(len(Netsetting.serverftp)>0 and len(Netsetting.userftp)>0 and len(Netsetting.passftp)>0):
                    ...
                    try:
                        ...
                        for files in glob.glob("*.txt"):
                            ...
                            print("List file .txt: "+path_file_ftp)
                            # Truyen file
                            Gsm.reportsms="FTP1:"+files+" "+ftp.storlines('STOR '+files ,open(path_file_ftp,'rb'))
                            print("Storlines FTP1:"+Gsm.reportsms)
                            # Xoa file
                            os.remove(path_file_ftp)
                            
                        ftp.quit()
                    except Exception as e:
                        Gsm.reportsms="Error FTP1:"+str(e)
            ...
            if(IOsetting.tinfor>0):
                ...
                if(counthour>=IOsetting.tinfor):
                    ...
                    if(len(Netsetting.serverftp2)>0 and len(Netsetting.userftp2)>0 and len(Netsetting.passftp2)>0):
                        ...
                        try:
                            ftp = FTP(Netsetting.serverftp2)
                            Gsm.reportsms=ftp.login(Netsetting.userftp2,Netsetting.passftp2)
                            result=ftp.cwd(Netsetting.pathftp2)
                            print("Login FTP2:"+Gsm.reportsms+" pathftp: "+ Netsetting.pathftp2)
                            #List ke file trong thu muc, gui file qua FTP2, roi xoa file do
                            os.chdir("/home/pi/bts/upload/")
                            filename_arr={}
                            i=0
                            for files in glob.glob("*.csv"):
                                ...
                                i= i+1
                                path_file_ftp="/home/pi/bts/upload/"+files
                                print("List file .csv: "+path_file_ftp)
                                Gsm.reportsms="FTP2:"+files+" "+ftp.storlines('STOR '+files ,open(path_file_ftp,'rb'))
                                print("Storlines FTP2:"+Gsm.reportsms)
                                # Xoa file
                                os.remove(path_file_ftp)
                                
                            ftp.quit()
                        except Exception as e:
                            Gsm.reportsms="Error FTP2:"+str(e)
        #-----------------------------------------------------------------------
        ...
        try:
            ...
            if(functionchannel==6):
                if(CHANGEOUTGENERATOR==1):
                    ...
                    self.instrument.write_register(startreg,int(Set.lowset[indexaddress]),numberreg,functionchannel,True)
                    self.data = int(Set.lowset[indexaddress])                

                elif(typereg==2):
                    print("write_longInt")
                    self.instrument.write_long(startreg,int(Set.lowset[indexaddress]),True)
                    self.data = int(Set.lowset[indexaddress])
                    
                elif(typereg==3):
                    print("write_float")
                    self.data =self.instrument.write_float(startreg,float(Set.lowset[indexaddress]),numberreg)
                    self.data = float(Set.lowset[indexaddress])
                    
                elif(typereg==4):
                    print("write_string")
                    self.data =self.instrument.write_string(startreg,str(Set.lowset[indexaddress]),numberreg)
                   

                elif(typereg==6):
                    print("write_longUint")
                    self.data =self.instrument.write_long(startreg,int(Set.lowset[indexaddress]),False)
